...
function removeUserMedia(userId) {
  const media = db.prepare("SELECT stored FROM media WHERE user_id=?").all(userId);
  for (const item of media) {
    try { fs.unlinkSync(path.join(UPLOAD_DIR, path.basename(item.stored))); } catch {}
  }
}

export function handleListUsers(res, sess) {
